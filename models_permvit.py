...
class Attention(nn.Module):
    """
    A simple modification of timm's Attention class to implement masked attention.
    """
...
class Block(nn.Module):
    """
    Simple modification of timm's Block to create a MaskedBlock
    """
...
class PermutationViT(nn.Module):
    """Permutation ViT, i.e. using permutations as our AR pretraining objective."""
    ...
    def create_permutation_mask(self, x):
        ...
        attn_mask = torch.gather(
            attn_mask, dim=2, index=idx_r)

        return attn_mask, ids_shuffle
    ...
    def forward_encoder(self, x):
        ...
        attn_mask, ids_shuffle = self.create_permutation_mask(x)
        ...
        # extend attention for cls token
    ...
    def forward_loss(self, imgs, pred, ids_shuffle):
        ...
        # permute target accordingly
        D = target.shape[-1]
        target = torch.gather(
            target, dim=1, index=ids_shuffle.unsqueeze(-1).repeat(1, 1, D)
        )
        ...
        # Mask for masking out the last token in perm, which can't be pred
        L = ids_shuffle.shape[1]
        mask = ids_shuffle != L - 1
        loss = (pred - target) ** 2
...
# Consider changing # of heads to 12
def perm_vit_tiny_patch16(**kwargs):
    model = PermutationViT(
        patch_size=16,
        embed_dim=192,
        depth=12,
        num_heads=3,
...
def perm_vit_small_patch16(**kwargs):
    model = PermutationViT(
        patch_size=16,
        embed_dim=384,
        depth=12,
...
def perm_vit_base_patch16(**kwargs):
    model = PermutationViT(
        patch_size=16,
        embed_dim=768,
        depth=12,
...
def perm_vit_large_patch16(**kwargs):
    model = PermutationViT(
        patch_size=16,
        embed_dim=1024,
        depth=24,
...
def perm_vit_huge_patch14(**kwargs):
    model = PermutationViT(
        patch_size=14,
        embed_dim=1280,
        depth=32,
